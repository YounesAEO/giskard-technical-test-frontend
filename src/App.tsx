...
function App() {
	...
	const saveChanges = async (e: any) => {
		...
		const limit = isChecked ? dateRange : null;
		const result = await request.post('/availabilities', {
			availabilities,
			limit,
		});
		console.log(result);
	...
	return (
		<div className="flex flex-col items-center justify-center h-screen gap-y-4">
			{days.map((day, i) => (
				<div key={i} className="w-1/2 flex gap-2 items-center">
					<div className="w-1/3 flex gap-2">
						<Toggle onToggle={(enabled) => onToggle(enabled, i)} />
						<label>{day}</label>
					</div>
			...
			<div className="flex gap-4 self-start ml-72">
